...
const getRouteFileMatchRegAll = function (config) {
    try {
        const jsonStr = Fs__default["default"].readFileSync(getPath([config.publicPath, './pages.json']), 'utf8');
        // 移除注释
...
function getConfigure(opts) {
    const config = Object.assign({}, Config, opts, {
        publicPath: publicPath + opts.publicPath
    });
...
function handleAppVue(source) {
    ...
    source += '<script>console.log(' + consoleStyle() + ')</script>';
